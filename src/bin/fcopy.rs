...
fn run() -> Result<(), String> {
    ...
    if args.is_empty() {
        return Err("fcopy: nothing to copy".to_string());
    }
    ...
    for item in items {
        println!("copied to clipboard: {}", item.path.display());
    }

    Ok(())
}

fn resolve_items(args: &[String]) -> Result<Vec<CopyItem>, String> {
    ...
    for arg in args {
        ...
        let absolute =
            absolute_path_preserving_final_symlink(&path).map_err(|err| match err.kind() {
                io::ErrorKind::NotFound => format!("fcopy: not found: {arg}"),
                _ => format!("fcopy: cannot resolve {arg}: {err}"),
            })?;
        let metadata = fs::symlink_metadata(&absolute).map_err(|err| match err.kind() {
            io::ErrorKind::NotFound => format!("fcopy: not found: {arg}"),
            _ => format!(
                "fcopy: cannot read metadata for {}: {err}",
                absolute.display()
            ),
        })?;
...
fn append_history(items: &[CopyItem]) -> Result<(), String> {
    let history = history_path()?;
    let parent = history
        .parent()
        .ok_or_else(|| format!("fcopy: invalid history path: {}", history.display()))?;

    fs::create_dir_all(parent).map_err(|err| {
        format!(
            "fcopy: cannot create history directory {}: {err}",
            parent.display()
        )
    })?;
    ...
    let mut file = OpenOptions::new()
        ...
        .truncate(false)
        .open(&history)
        .map_err(|err| format!("fcopy: cannot open history {}: {err}", history.display()))?;

    file.lock_exclusive()
        .map_err(|err| format!("fcopy: cannot lock history {}: {err}", history.display()))?;
    ...
    write_result
        .map_err(|err| format!("fcopy: cannot write history {}: {err}", history.display()))?;
    unlock_result
        .map_err(|err| format!("fcopy: cannot unlock history {}: {err}", history.display()))?;
...
fn history_line(items: &[CopyItem]) -> String {
    ...
    json.push_str(",\"operation\":\"copy\",\"created_at\":");
...
fn history_path() -> Result<PathBuf, String> {
    ...
    Err(
        "fcopy: cannot determine history directory: LOCALAPPDATA and USERPROFILE are not set"
            .to_string(),
    )
}
...
fn history_path() -> Result<PathBuf, String> {
    let home = non_empty_env("HOME")
        .ok_or_else(|| "fcopy: cannot determine history directory: HOME is not set".to_string())?;
...
mod tests {
    ...
    fn file_type_name_reports_symlink() {
        ...
        let dir = env::temp_dir().join(format!("fcopy-test-{}", unique_id(SystemTime::now())));
